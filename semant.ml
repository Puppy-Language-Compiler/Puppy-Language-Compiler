...
let rec transExp venv tenv exp =
  ...
  | A.OpExp {left; op; right; pos} -> (
      let l = trexp left and r = trexp right in (
      match op with
  ...
  | A.SeqExp exps -> List.fold_left (fun _ exp -> trexp exp) {exp = (); ty = Types.UNIT} (List.map fst exps)
  | A.IfExp {test; then'; else'; pos} -> {exp = (); ty = trif venv tenv test then' else'}
  | A.WhileExp {test; body; pos} ->
      (trwhile venv tenv test body;
       {exp = (); ty = Types.UNIT})
  | A.ForExp {var; lo; hi; body; pos; _} ->
    (trfor venv tenv var lo hi body;
    {exp = (); ty = Types.UNIT})
  | A.BreakExp pos -> {exp = (); ty = Types.UNIT}
  | A.ArrayExp {typ = typid; size; init; pos} -> {exp = (); ty = trarray venv tenv typid size init}
...
and trlval l = (
      ...
      | A.ArrayAccess {array; exp; pos} -> (
          ...
          let {exp = _; ty = indexty} = trexp exp in
            match arrty with
...
and trcall venv tenv func args =
  ...
  | Some (Env.FunEntry {formals = formals_ty; result}) ->
    let args_ty = List.map (fun exp -> (trexp exp).ty) args in
    List.iter2
      (fun expected actual ->
...
and trrecord venv tenv rec_fields typ_id =
  ...
  match rec_type with
  | Types.RECORD (fields, unique) ->
      let rec_act_ty = List.map (fun (symbol, exp, _) -> (trexp exp).ty) rec_fields in
...
(* 检查if-then或if-then-else表达式
 * if-then: 检查if条件是否为int类型，检查then表达式的返回类型是否为Types.UNIT
 * if-then-else: 检查if条件是否为int类型，检查then和else的类型是否一致
 *)
and trif venv tenv test then' else' =
  let {exp = _; ty = test_ty} = trexp test in
  if Types.check_ty Types.INT test_ty then
  match else' with
  | None ->
      let {exp = _; ty = then_ty} = trexp then' in
      if Types.check_ty Types.UNIT then_ty then
        Types.UNIT
      else
        failwith "This expression should has type unit, cause it is if-then expression."
  | Some else_exp ->
      let {exp = _; ty = then_ty} = trexp then' in
      let {exp = _; ty = else_ty} = trexp else_exp in
      if Types.check_ty then_ty else_ty then then_ty
      else failwith "The types of then and else expression are not matched."
  else
    failwith "The type of if condition is expected int."

(* 检查while表达式
 * while的条件需要返回int类型
 * 循环体需要返回unit类型
 *)
and trwhile venv tenv test body =
  let test_ty = (trexp test).ty in
  let body_ty = (trexp body).ty in
  if Types.check_ty Types.INT test_ty then
    if not (Types.check_ty Types.UNIT body_ty) then
      failwith "The type of the body should be unit."
  else
    failwith "The type of condition should be int."

(* 检查for表达式
 * 检查lo域、hi域是否为int
 * 检查body域，并判断返回的类型是否为Types.UNIT
 *)
and trfor venv tenv var lo hi body =
  let lo_ty = (trexp lo).ty in
  let hi_ty = (trexp hi).ty in
  let body_ty = (trexp body).ty in
  if (not (Types.check_ty Types.INT lo_ty)) or (not (Types.check_ty Types.INT hi_ty)) then
    failwith "The border of for expression should be type of int."
  else
    if not (Types.check_ty Types.UNIT body_ty) then
      failwith "The body of for expression should return type unit."

(* 检查数组表达式ArrayExp
 * 在类型环境tenv中检查typid是否存在，若存在，返回其绑定的类型type
 * 判断type是否为Types.ARRAY (ty, unique)类型，若是
 * 检查size是否为Types.INT类型
 * 检查init的类型是否与type相一致
 * 最终返回数组的类型
 *)
and trarray venv tenv typid size init =
  let arr_ty = find_ty tenv typid in
  match arr_ty with
  | Types.ARRAY (ty, _) ->
      let size_ty = (trexp size).ty in
      if not (Types.check_ty Types.INT size_ty) then
        failwith "The size of array is not a type of int."
      else
        let init_ty = (trexp init).ty in
        if Types.check_ty arr_ty init_ty then
          arr_ty
        else
          failwith "The types of initialization and array are not matched."
  | _ -> failwith (sprintf "The %s is not a type of array." (Symbol.name typid))
